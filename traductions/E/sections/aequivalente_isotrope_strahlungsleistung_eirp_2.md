...
où $G_\mathrm{Antenne}$ représente ici le gain d'antenne. L'EIRP est donc le produit de la puissance fournie directement à l'antenne par son gain dans une direction, rapporté au radiateur isotrope.
...
Dans la question suivante, il faut impérativement faire attention aux signes des opérations. Les pertes sont *soustraites* de la puissance d'émission, puis le résultat est *multiplié* par le facteur de gain ($G_\mathrm{Antenne}$). Comme c'est l'EIRP qui doit être calculée, la référence doit être le radiateur isotrope.
...
<indepth>
On obtient la formule de $P_\mathrm{EIRP}$ en réarrangeant en conséquence la formule du gain du recueil de formules :
  
$g = 10 \cdot \log_{10}\left(\frac{P_2}{P_1}\right) \unit{\dB}$
  
Comme il faut en outre tenir compte d'une atténuation $a$, celle-ci est soustraite du gain d'antenne. Pour $P_1$, nous insérons la puissance de l'émetteur $P_\mathrm{Sender}$, puisqu'elle représente la puissance d'entrée, et pour $P_2$ en conséquence $P_\mathrm{EIRP}$, puisqu'il s'agit de la puissance de sortie résultante.
...
Par multiplication par $P_\mathrm{Sender}$, on obtient la formule recherchée :
...
$P_\mathrm{EIRP} = P_\mathrm{Sender} \cdot 10^{\frac{g_i-a}{\qty{10}{\dB}}}= \qty{250}{\milli\watt} \cdot 10^{\frac{\qty{26}{\dBi}}{\qty{10}{\dB}}}= \qty{250}{\milli\watt} \cdot 398 \approx \qty{100}{\watt}$
...
Ici aussi, il y a de nouveau deux méthodes de calcul possibles. Commençons par la méthode utilisant la formule connue. Dans cet exercice, ce n'est toutefois pas la puissance rayonnée $P_\mathrm{EIRP}$ qui est recherchée, mais la puissance d'émission $P_\mathrm{Sender}$. Nous devons donc réarranger la formule en conséquence :

$P_\mathrm{EIRP} = P_\mathrm{Sender} \cdot 10^{\frac{g_i-a}{\qty{10}{\dB}}} \quad\quad\quad | : 10^{\frac{g_i-a}{\qty{10}{\dB}}}$

On obtient ainsi :

$ P_\mathrm{Sender} = \frac{P_\mathrm{EIRP}}{10^{\frac{g_i-a}{\qty{10}{\dB}}}} $

Nous insérons les valeurs :

$ P_\mathrm{Sender} = \frac{\qty{10}{\watt}}{10^{\frac{\qty{5,15}{\dBi}}{\qty{10}{\dB}}}} = \frac{\qty{10}{\watt}}{3,27} \approx \qty{3,05}{\watt} $
...
$P_\mathrm{Sender} = \frac{\qty{10}{\watt}}{2\cdot 1,64} = \frac{\qty{10}{\watt}}{3,28} \approx \qty{3}{\watt}$
...
où $P_\mathrm{ERP}$ représente la puissance rayonnée rapportée au dipôle. 
...
On cherche la puissance isotrope rayonnée équivalente $P_\mathrm{EIRP}$. Il faut d'abord tenir compte de l'atténuation de câble. Une atténuation de $\qty{10}{\dB}$ correspond à un rapport de puissance de $\num{0,1}$. Avec ce facteur d'atténuation ainsi que le facteur de gain d'antenne du dipôle de $\num{1,64}$, on peut ensuite calculer la puissance rayonnée.

$P_\mathrm{EIRP} = \qty{100}{\watt} \cdot 0,1 \cdot 1,64 = \qty{16,4}{\watt}$


Pour la question suivante, le recueil de formules contient aussi directement une formule applicable. Comme nous avons une antenne directive dont le gain est indiqué par rapport au dipôle (ERP), il faut encore ajouter $\qty{2,15}{\dB}$ pour le calcul de $P_\mathrm{EIRP}$ :
