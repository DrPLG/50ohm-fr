...
\begin{circuitikz}
    ...
    \draw(h1.center)  node[anchor=north] (h11)  {\footnotesize Câble 1};
    \draw(h11.south)  node[anchor=north] (h111) {\footnotesize 2\,dB};
    \draw(h111.south) node[anchor=north] ()     {\footnotesize Atténuation};

    \draw(amp.north)  node[anchor=south] (amp1) {\footnotesize 6\,dB de gain};
    \draw(amp1.north) node[anchor=south] () {\footnotesize PA};

    \draw(h2.center)  node[anchor=north] (h22)  {\footnotesize Câble 2};
    \draw(h22.south)  node[anchor=north] (h222) {\footnotesize 3\,dB};
    \draw(h222.south) node[anchor=north] ()     {\footnotesize Atténuation};
    ...
    \draw(h333.south west) node[anchor=north west] (h333) {\footnotesize Gain};
