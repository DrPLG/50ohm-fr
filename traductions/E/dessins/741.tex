...
\begin{circuitikz}[european]
    ...
    %
    \draw(tx.left) node[below left]() {Émetteur};
    \draw(rx.right) node[below right]() {Récepteur};
    %
    \draw(c4) to [open, name={h1}] (c2);
    \draw[DARCred](h1.center) node[above, rotate=-44](){\small~~~Onde d'espace};
    %
    \draw[DARCgreen](end) node[above](){\footnotesize\quad\quad~~Onde de sol};
    %
    \draw(c2) node[](){|};
    \draw(c5) node[rotate=30](){|};
    \draw(-0.25,2.25) node[](){Zone de silence};
    %
\end{circuitikz}