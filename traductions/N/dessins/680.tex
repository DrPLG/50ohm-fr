...
\begin{tikzpicture}
    ...
    \draw(netzteil.north) ++(0,0.6) node[anchor=south]{Alimentation secteur};
    ...
    \draw[red](netzteil.plus)
        to [fuse, l={Fusible}, color=red] ++(2.5,0)
        to [short, i={Courant continu (I)}, color=red] (trx.plus);
    \draw(trx.minus) to [short, i={}, l={\textcolor{gray}{\small{\textit{\parbox{4.4cm}{\centering Risque de court-circuit\\ et d'inversion de polarité}}}}}] (netzteil.minus);
    ...
    \draw[gray](c1) to [open] ++(-0.5,0.4) node[](c3){\itshape\small Haute tension};
\end{tikzpicture}